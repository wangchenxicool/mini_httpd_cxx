...
std::string Json::Value::escapeJsonString (const std::string& _input) {
    ...
    //XXX change by wcx
    //for (string::iterator iter = (char*) input.begin (); iter != input.end(); iter++) {
...
std::string Json::Value::toString (const unsigned int value)  {
    char buf[20];

    _snprintf (buf, 20, "%u", value);

    return buf;
}
std::string Json::Value::toString (const double value)  {
    char buf[20];
    _snprintf (buf, 20, "%0.4f", (double) value);
    return buf;
}
...
void Json::Object::append (const Json::Object& obj) {
    ...
    for (std::map< Key, Value >::const_iterator cit = obj._memberMap.begin(); cit != obj._memberMap.end(); ++cit) {
...
const Json::Value& Json::Object::operator[] (const std::string& key) {
...
bool Json::Object::contain (const Object& o) const {
    ...
    for (std::map< Key, Value >::const_iterator it = o._memberMap.begin(); it != o._memberMap.end(); ++it) {
        //for(const std::pair< Key, Value >& p : o._memberMap)
        //for(const std::pair< Key, Value >& p : o._memberMap)
        {
            std::map< Key, Value >::const_iterator cit = _memberMap.find (it->first);
...
bool Json::Array::operator== (const Array& a) const {
    ...
    for (std::list< Value >::const_iterator v0 = a._elementList.begin(); v0 != a._elementList.end(); v0++)
        //for(const Value& v0 : a._elementList)
    {
        bool found = false;
        for (std::list< Value >::const_iterator v1 = _elementList.begin(); v1 != _elementList.end(); v1++)
